...
// Create a Listener "component" that subclasses the generic rclcpp::Node base
...
Listener::Listener(const rclcpp::NodeOptions &options)
  ...
  std::cout << "############## Address of listener singleton: " << singleton
            << std::endl;

  // Create a subscription to the "chatter" topic which can be matched with one
  // or more compatible ROS publishers. Note that not all publishers on the same
  // topic with the same type will be compatible: they must have compatible
